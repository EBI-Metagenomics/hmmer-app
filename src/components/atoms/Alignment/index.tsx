...
export const Alignment: React.FC<AlignmentProps> = ({
    ...
    const { hmmname, seqname } =
        algorithm === "hmmscan" ? { hmmname: "Target", seqname: "Query" } : { hmmname: "Query", seqname: "Target" };
    ...
    return (
        <pre>
            {!alignmentOnly && <div className="alimeta">{/* Meta information table here */}</div>}
            <div className="alignment vf-font-plex-mono">
                {!included && <div>Insignificant match:</div>}
                {blocks.map((block, blockIndex) => (
                    <AlignmentBlockComponent key={blockIndex} block={block} hmmname={hmmname} seqname={seqname} />
                ))}
            </div>
        </pre>
    );
